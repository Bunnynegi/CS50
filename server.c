...
bool load(FILE* file, BYTE** content, size_t* length)
{
    *content = NULL;
    *length = 0;
    char* filenew = malloc(sizeof(char));
    if (filenew == NULL) 
    {
        return false; 
    }
    ...
    for (int c = fgetc(file); c != EOF; c = fgetc(file))
    {
        filenew[i] = (char) c;
        i++;
        filenew = (char*) realloc(filenew, sizeof(char)*(i+1));
    }
    *content = filenew;
    *length = i;
    return true;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char *version=calloc(1,LimitRequestFields+1);
    char *reqtar=calloc(1,LimitRequestFields+1);
    char *method=calloc(1,LimitRequestFields+1);
    int m=0,j=0,k=0;
